...
const signupSchema = joi.object({
    ...
    firstname: joi.string().min(3).max(30).required().messages({
      ...
      'string.min': `Le prénom doit comporter au minimum {#limit} caractères`,
      'string.max': `Le prénom doit comporter au maximum {#limit} caractères`,
    }),
    lastname: joi.string().min(3).max(30).required().messages({
      'string.empty': `Le champs nom ne peut pas être vide.`,
      'any.required': `Le champs nom est obligatoire.`,
      'string.min': `Le nom doit comporter au minimum {#limit} caractères`,
      'string.max': `Le nom doit comporter au maximum {#limit} caractères`,
    }),
